...
```{r}
load("input/data/original/ELSOC_W05_v1.0_R.RData")
...
# Cargar datos territoriales ELSOC 2018
```{r}
data_territorial <- read_excel("input/data/original/BASE_2018_COD.xlsx")

data_territorial <- data_territorial %>% dplyr::select(idencuesta, trabaja, cesante, inactivo, jh_esc_sd) %>% as.data.frame(.)

data_territorial <- data_territorial %>%  set_na(trabaja, cesante, inactivo, jh_esc_sd, na = -995, drop.levels = TRUE, as.tag = FALSE) # recode missings de no respuesta a NA

data <- inner_join(data, data_territorial, by="idencuesta", copy = FALSE, suffix = c("", "_ter"))
```

# Cargar datos territoriales CIT 2016
```{r}
cit <- read_excel("input/data/original/ELSOC_CIT_Dataset_2016.xlsx")

cit <- cit %>% dplyr::select(idencuesta, acver_mnz, aced_mnz, accs_mnz) %>% as.data.frame()

cit <- cit %>% set_na(acver_mnz, aced_com, accs_mnz, na = c(994, 995,996), drop.levels = TRUE, as.tag = FALSE) # recode missings de no respuesta a NA
...
data$m01_w01 <- car::recode(data$m01_w01, "c(1,2,3,4)=1;c(5)=2;c(6,7)=3;c(8,9,10)=4")
data$m01_w02 <- car::recode(data$m01_w02, "c(1,2,3,4)=1;c(5)=2;c(6,7)=3;c(8,9,10)=4")
data$m01_w03 <- car::recode(data$m01_w03, "c(1,2,3,4)=1;c(5)=2;c(6,7)=3;c(8,9,10)=4")
data$m01_w04 <- car::recode(data$m01_w04, "c(1,2,3,4)=1;c(5)=2;c(6,7)=3;c(8,9,10)=4")
data$m01_w05 <- car::recode(data$m01_w05, "c(1,2,3,4)=1;c(5)=2;c(6,7)=3;c(8,9,10)=4")

data$m01_w01 <- factor(data$m01_w01, levels = c("1", "2", "3", "4"), labels=c( "Menos que media completa", "Media completa", "Educacion tecnica superior", "Educacion universitaria y Postgrado"))
data$m01_w02 <- factor(data$m01_w02, levels = c("1", "2", "3", "4"), labels=c( "Menos que media completa", "Media completa", "Educacion tecnica superior", "Educacion universitaria y Postgrado"))
data$m01_w03 <- factor(data$m01_w03, levels = c("1", "2", "3", "4"), labels=c( "Menos que media completa", "Media completa", "Educacion tecnica superior", "Educacion universitaria y Postgrado"))
data$m01_w04 <- factor(data$m01_w04, levels = c("1", "2", "3", "4"), labels=c( "Menos que media completa", "Media completa", "Educacion tecnica superior", "Educacion universitaria y Postgrado"))
data$m01_w05 <- factor(data$m01_w05, levels = c("1", "2", "3", "4"), labels=c( "Menos que media completa", "Media completa", "Educacion tecnica superior", "Educacion universitaria y Postgrado"))
...
data %>% select(m29_w01, m29_w02, m29_w03, m29_w04, m29_w05)  %>%  dfSummary(, graph.col = FALSE)
```
...
```{r}
data <- data %>% dplyr::mutate(quintiles_ingresos_w01 = ntile(m29_w01, 5))
...
data$quintiles_ingresos_w01 <- factor(data$quintiles_ingresos_w01, levels = c(1,2,3,4,5), labels = c("Quintil 1", "Quintil 2", "Quintil 3", "Quintil 4", "Quintil 5"))
data$quintiles_ingresos_w02 <- factor(data$quintiles_ingresos_w02, levels = c(1,2,3,4,5), labels = c("Quintil 1", "Quintil 2", "Quintil 3", "Quintil 4", "Quintil 5"))
data$quintiles_ingresos_w03 <- factor(data$quintiles_ingresos_w03, levels = c(1,2,3,4,5), labels = c("Quintil 1", "Quintil 2", "Quintil 3", "Quintil 4", "Quintil 5"))
data$quintiles_ingresos_w04 <- factor(data$quintiles_ingresos_w04, levels = c(1,2,3,4,5), labels = c("Quintil 1", "Quintil 2", "Quintil 3", "Quintil 4", "Quintil 5"))
data$quintiles_ingresos_w05 <- factor(data$quintiles_ingresos_w05, levels = c(1,2,3,4,5), labels = c("Quintil 1", "Quintil 2", "Quintil 3", "Quintil 4", "Quintil 5"))

data %>% select(quintiles_ingresos_w01, quintiles_ingresos_w02, quintiles_ingresos_w03, quintiles_ingresos_w04, quintiles_ingresos_w05) %>% sjlabelled::as_label(.) %>% summarytools::dfSummary(, graph.col = FALSE)
```
...
## Informales. cumple cualquiera de las siguientes condiciones
## catocup 1, 2, 3 o 7 + m10==3 sin contrato escrito
## FNR catocup== 6
## catocup 4, 5 + m11 = 3 no da boleta ni factura
## catocup 1, 2, 3 o 7 + m18==4 no cotiza
...
```{r}
# Se recodifican a quintiles y se dicotomizan en cuartil 4
data <- data %>% dplyr::mutate(percent_sded = ntile(jh_esc_sd, 4))
data <- data %>% dplyr::mutate(percent_trabaja = ntile(trabaja, 4))
data <- data %>% dplyr::mutate(percent_cesante = ntile(cesante, 4))
data <- data %>% dplyr::mutate(percent_inactivo = ntile(inactivo, 4))
data <- data %>% dplyr::mutate(percent_acver = ntile(acver_mnz, 4))
data <- data %>% dplyr::mutate(percent_aced = ntile(aced_mnz, 4))
data <- data %>% dplyr::mutate(percent_accs = ntile(accs_mnz, 4))
data$percent_sded <- car::recode(data$percent_sded, "c(1,2,3)=1; 4=2")
data$percent_trabaja <- car::recode(data$percent_trabaja, "c(1,2,3)=1; 4=2")
data$percent_cesante <- car::recode(data$percent_cesante, "c(1,2,3)=1; 4=2")
data$percent_inactivo <- car::recode(data$percent_inactivo, "c(1,2,3)=1; 4=2")
data$percent_acver<- car::recode(data$percent_acver, "c(1,2,3)=1; 4=2")
data$percent_aced <- car::recode(data$percent_aced, "c(1,2,3)=1; 4=2")
data$percent_accs <- car::recode(data$percent_accs, "c(1,2,3)=1; 4=2")
```
